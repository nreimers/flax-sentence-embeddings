...
class UtilTest(unittest.TestCase):
    ...
    def test_normalize_L2(self):
        """Tests the correct computation of normalize_L2"""
        a = onp.random.randn(50, 100)

        pytorch_normalize = torch_F.normalize(torch.tensor(a), p=2, dim=1).numpy()
        jax_normalize = onp.asarray(jax_util.normalize_L2(a))

        assert pytorch_normalize.shape == jax_normalize.shape
        for i in range(len(pytorch_normalize)):
            assert onp.all(onp.abs(pytorch_normalize[i] - jax_normalize[i]) < 0.001), "Output : torch - {}, jax - {}" \
                .format(pytorch_normalize[i], jax_normalize[i])
